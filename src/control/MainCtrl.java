	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("login"))
		{
			    ...
			    String userpwd = request.getParameter("userpwd");
			    	...
			    	gor("/wsbooksys/admin/index.jsp", request, response);
			    	}else{
...
//			go("/tocar.jsp?gid="+gid, request, response);
		...
		if(ac.equals("tobuy")){
			...
			if(member!=null){
				...
				ArrayList<HashMap> addrlist = (ArrayList<HashMap>)dao.select("select * from addr where delstatus='0' and memberid="+member.get("id"));
				if(addrlist.size()==0){
					out.print("false");
				}else{
...
//			go("/tocar.jsp?gid="+gid, request, response);
		...
		if(ac.equals("ddfh")){
			...
			if(flag){
				...
				request.setAttribute("info", "订单中图书库存不足，发货失败!");
				go("/admin/ddgl.jsp", request, response);
			}
		}
		...
		if(ac.equals("register")){
				...
				dao.commOper("insert into member (uname,upass,email,tname,sex,addr,ybcode,qq,tel,delstatus,savetime)" +
							" values ('"+uname+"','"+upass+"','"+email+"','"+tname+"','"+sex+"','"+addr+"','"+ybcode+"','"+qq+"','"+tel+"','"+delstatus+"','"+savetime+"')");
		...
		if(ac.equals("goodsedit")){
			...
			dao.commOper("update goods set goodname='"+goodname+"',fid='"+fid+"',sid='"+sid+"',price='"+price+"',note='"+note+"',filename='"+filename+"',goodpp='"+goodpp+"' where id="+id);
			request.setAttribute("suc", "操作成功!");
		...
		if(ac.equals("pj")){
			...
			request.setAttribute("suc", "图书评价成功!");
			go("/mydd.jsp", request, response);
		}
		//管理员回复评价
		if(ac.equals("pjhf")){
		...
		if(ac.equals("addfav")){
			...
			if(member==null){
				...
				if(cklist.size()==0){
					...
					request.setAttribute("suc", "此图书已收藏过!");
					go("/"+page, request, response);
				}
			}
		}
		...
		if(ac.equals("delfav")){
			String goodid = request.getParameter("goodid");
			String page = request.getParameter("page");
			dao.commOper("delete from fav where memberid='"+member.get("id")+"' and goodid='"+goodid+"'");
		...
		if(ac.equals("addradd")){
			...
			dao.commOper("insert into addr (memberid,shr,shrtel,shraddr,delstatus) values " +
					"('"+memberid+"','"+shr+"','"+shrtel+"','"+shraddr+"','"+delstatus+"') ");
			request.setAttribute("suc", "操作成功!");
